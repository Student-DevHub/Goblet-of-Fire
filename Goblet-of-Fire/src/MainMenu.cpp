...
namespace GobletOfFire {
  namespace Scene {
    MainMenu::MainMenu(const std::shared_ptr<Input::InputManager> &input_manager,
      const std::shared_ptr<Core::CoreEngine> &engine)
      : input_manager_(input_manager), local_buffer_(std::make_shared<Graphics::buffer>()),
        engine_(engine), background_(nullptr),
    ...
    void MainMenu::createCallBacks() {
      ...
      option_callbacks_.insert(std::make_pair(0, stop));
